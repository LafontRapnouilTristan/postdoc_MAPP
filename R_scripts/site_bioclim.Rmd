...
var_cor <- GGally::ggpairs(
    ...
    progress = T,
    cardinality_threshold = 105)
ggsave("../Figures/cov_cor_mean_3.png",var_cor,height = 15,width = 15)
```

## Histograms covariates

```{r}
vars <- c("X","Y","bulk","gHM","height","ph","soc","water",colnames(data_sat)[grep('_lt', names(data_sat))])

for(i in vars){
  ggsave(plot=ggplot(mean_month_df,aes(x=mean_month_df[,i]))+
             geom_histogram()+
             theme_classic2()+
             xlab(i),filename=paste0("../Figures/cov_histogram/",i,".png"))
}
```
