...
```{r}
EcophyCofog::Library(c("EcophyCofog","metabaR","readr","dplyr","magrittr","ggplot2","reshape2","patchwork","ggpubr","tidyr","forcats","phyloseq","cowplot","readxl","microeco","file2meco","RColorBrewer","vegan","sf","rnaturalearth","rnaturalearthdata","dendextend","GGally"))
```
...
for(i in formulaZ){
    ...
    dbrda_site_biplot[[formula_tmp]] <- ggplot(fort_rda_tmp, aes(x = dbRDA1, y = dbRDA2)) +
        geom_point(data = cbind(subset(fort_rda_tmp, score == "sites"), # add points (sites) corresponding to samples
                                ...
                                cluster=combined$bp_st_aft), 
                   aes(color = country, shape = combined$bp_st_aft), 
                   size = 3) +
...
```{r}
picrust_closestref <-  read_delim("picrust2/Galaxy177-[FROGSFUNC_1_placeseqs_and_copynumbers__frogsfunc_placeseqs_closests_ref_sequences.txt].tsv", # read the tsv file
                            delim = "\t", escape_double = FALSE, 
                            trim_ws = TRUE)
```
...
nrow(picrust_closestref) # 21591 ASVs in picrust
...
Additionnaly, a coverage < 80% is pretty low 
...
asvs_picrust <- asvs_picrust$`#ASV` #20179 asvs
discarded_picrust <- discarded_picrust$`#ASV` # 1412 asvs
```

Set cov and Id to 0.8 in galaxy FrogsFunc 2.

Investigate Excluded ASVs and their repartition across my samples.
297 ASVs are exluded with this filtering
...
It changes a bit my community composition?
