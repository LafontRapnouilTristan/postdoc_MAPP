...
get_density <- function(x, y, ...) {
  ...
  ii <- cbind(ix, iy)
  return(dens$z[ii])
>>>>>>> 05b2926576e87756932052c037b9cf23240849c3
}
```
...
```{r}
rm(alpha_plot,alpha,alpha_plot_pct,data_temp,df_tmp,donut_comp,meco_16s_singl,plot_data,singleton_plots,singletons,n_non_single_asv,n_non_single_reads,n_single_asv,n_single_reads,non_single_asv,single_asv,use_taxanames)
```
...
```{r}
rm(ctest,mds_16s,mds_filtered,mds_ns,p1,p2,p1.solution,list_pro_title,pro_plot_com,pro_tmp,pro.error,procr,procrustes_res,list_procrustes_com,t1,t2,t3,quantiles)
```
...
for(i in c("meco_16s_ns","meco_16s_relabfiltered")){
  ...
  df_ordi_sites %<>%
      ...
      left_join(site_bp)%>%
    mutate(bp_lt=as.factor(bp_lt),
           bp_st=as.factor(bp_st))
  
  for(j in c("bp_lt","bp_st")){  
    
    ordi_plots_sites[[i]][[j]] <-  ggplot(df_ordi_sites,aes(x=MDS1,y=MDS2,color=!!df_ordi_sites[,j]))+
      geom_point()+
      theme_dark()+
      # geom_text()+
      scale_color_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080'),
...
(meco_16s_ns$otu_table%>%
    ...
    reshape2::melt()%>%
    mutate(variable=gsub("[A-Z]|-|(?=_).*","",variable,perl = T))%>%
    ggplot(aes(x=variable,y=otus,fill=log(value)))+
    geom_tile()+
    scale_fill_viridis_c()+
    theme_void())/(
      meco_16s_ns$sample_table%>%
...
```{r}
mixomics_plotlists[[i]][["final"]]
plotVar(mixomics_plotlists[[i]][["final"]],cutoff = .3)
```
...
net1 <- trans_network$new(dataset = meco_16s_relabfiltered, cor_method = "pearson",use_WGCNA_pearson_spearman=T)
...
for(i in c("lt","st")){
  ...
  for(j in c("filt","nonfilt")){
    ...
    gdm_plot_var[[i]][[j]] <- gdm_spline_data$x%>%
      reshape2::melt()%>%
      rename(valuex=value)%>%
      left_join(reshape2::melt(gdm_spline_data$y[,which(colSums(gdm_spline_data$y)!=0)]))%>%
      rename(valuey=value)%>%
      filter(Var2%in%names(which(colSums(gdm_spline_data$y)!=0)))%>%
...
var_effect_lt<- gdm_spline_lt$x%>%
    reshape2::melt()%>%
    rename(valuex=value)%>%
    left_join(reshape2::melt(gdm_spline_lt$y))%>%
    mutate(Var2=gsub("_lt","",Var2))%>%
    rename(valuey=value)%>%
    ...
    theme_bw()+
    ylab("Biological change")+
    xlab("Variable gradient")
...
predsobs_plot <- ggplot(prd_df,
    ...
    geom_smooth(method="lm")+ 
    viridis::scale_color_viridis()+
    ylab("Predicted biological distance")+
    xlab("Observed biological distance")
>>>>>>> 05b2926576e87756932052c037b9cf23240849c3
```
...
df_glmm_tmp <- cbind(p, nb_asvs=combined$nb_asvs)

df_glmm_tmp%>%
  ggplot(aes(x=nb_asvs,y=estimate))+
  geom_point()
```
...
```{r}
mat_dist <- codep::geodesics(combined[,c("X","Y")])
mat_geo <- geodist(combined[,c("X","Y")],measure = 'geodesic')
# class(mat_geo) <- c("matrix","dist")
mems <- dbmem(as.dist(mat_geo))
mems2 <- dbmem(mat_dist)


mems$Lat <- combined$Y
mems$Lon <- combined$X
mems$Lon[which(duplicated(mems$Lon))] <- jitter(mems$Lon[which(duplicated(mems$Lon))] ,amount = 8.983112*10^-6)


mems2$Lat <- combined$Y
mems2$Lon <- combined$X
mems2$Lon[which(duplicated(mems2$Lon))] <- jitter(mems2$Lon[which(duplicated(mems2$Lon))] ,amount = 8.983112*10^-6)
...
mem_long_sf <- st_as_sf(mem_long, coords = c("Lon","Lat"),crs = 4326, agr = "constant")


ggplot() +
  geom_sf(data = world_lambert, fill = "grey", color = "darkgrey", size = 0.1)+
  geom_sf(data=filter(mem_long_sf,grepl("M10|M2",MEM)), aes(fill=Value),shape=23)+
  facet_wrap(~MEM)+
  ggtitle("MAPP 16s sites") +
...
mem_long <- gather(mems2, MEM, Value, MEM1:(ncol(mems2)-2))
mem_long_sf <- st_as_sf(mem_long, coords = c("Lon","Lat"),crs = 4326, agr = "constant")


ggplot() +
  geom_sf(data = world_lambert, fill = "grey", color = "darkgrey", size = 0.1)+
  geom_sf(data=filter(mem_long_sf,grepl("M8|M9",MEM)), aes(fill=Value),shape=23)+
  facet_wrap(~MEM)+
  ggtitle("MAPP 16s sites") +
  theme(plot.title = element_text(hjust = 0.5))+
  theme_minimal()+
...
```{r}
mst <- spdep::mstree(attr(mems,"listw"))
mst
plot(mst,st_coordinates(mxy))


new_nb <- mst.nb(codep::geodesics(combined[,c("X","Y")]))
```


```{r}
nbtri <- tri2nb(st_coordinates(mxy))
nbgab <- graph2nb(gabrielneigh(st_coordinates(mxy)), sym = TRUE)
nbrel <- graph2nb(relativeneigh(st_coordinates(mxy)), sym = TRUE)

ptmplist <- NULL
for(i in c("nbtri","nbgab","nbrel")){
  
  tmp <- get(i)
  
  euch <- nb2lines(tmp,coords=st_coordinates(sites2),as_sf = T)
  st_crs(euch) <- 4326
  
  # plot
  ptmp<-ggplot() +
    geom_sf(data = world_lambert, fill = "grey", color = "darkgrey", size = 0.1)+
    geom_sf(data=sites2, pch=19, color="blue") +
    geom_sf(data=euch, color="blue", lwd=0.3, lty=2) +
    labs(title=paste0("Adjacency Connection: ",i)) + 
    xlab("") +
    ylab("")
  ptmplist[[i]] <- ptmp
}

listw2lines(attr(mems,"listw"),coords=st_coordinates(sites2),as_sf = T)

ptmplist$nbtri/ptmplist$nbgab/ptmplist$nbrel
```

```{r}
youchi <- listw2lines(attr(mems,"listw"),coords=st_coordinates(sites2),as_sf = T)
st_crs(youchi) <- 4326

youchado <- listw2lines(attr(mems2,"listw"),coords=st_coordinates(sites2),as_sf = T)
st_crs(youchado) <- 4326


youchar <- listw2lines(nb2listw(new_nb),coords=st_coordinates(sites2),as_sf = T)
st_crs(youchar) <- 4326

ggplot() +
  geom_sf(data = world_lambert, fill = "grey", color = "darkgrey", size = 0.1)+
  geom_sf(data=sites2, pch=19, color="blue") +
  geom_sf(data=youchar, color="blue", lwd=0.3, lty=2) +
  labs(title=paste0("Adjacency Connection: ","mst")) + 
  xlab("") +
  ylab("")+
  ggplot() +
  geom_sf(data = world_lambert, fill = "grey", color = "darkgrey", size = 0.1)+
  geom_sf(data=sites2, pch=19, color="blue") +
  geom_sf(data=youchi, color="blue", lwd=0.3, lty=2) +
  labs(title=paste0("Adjacency Connection: ","mems")) + 
  xlab("") +
  ylab("")+
  ggplot() +
  geom_sf(data = world_lambert, fill = "grey", color = "darkgrey", size = 0.1)+
  geom_sf(data=sites2, pch=19, color="blue") +
  geom_sf(data=youchado, color="blue", lwd=0.3, lty=2) +
  labs(title=paste0("Adjacency Connection: ","mems2")) + 
  xlab("") +
  ylab("")
```
...
```{r}
nbdists(new_nb,coords=sites2)%>%unlist%>%max;give.thresh(mat_dist);give.thresh(mat_geo)
```

```{r}
st_distance(mem_long_sf,mem_long_sf,which="Great Circle")%>%max();max(mat_geo)
```


```{r}
mems_shorter <- dbmem(mat_dist, thresh = 7549762)


youchi_shorter <- listw2lines(attr(mems_shorter,"listw"),coords=st_coordinates(sites2),as_sf = T)
st_crs(youchi_shorter) <- 4326

ggplot() +
  geom_sf(data = world_lambert, fill = "grey", color = "darkgrey", size = 0.1)+
  geom_sf(data=sites2, pch=19, color="blue") +
  geom_sf(data=youchi_shorter, color="blue", lwd=0.3, lty=2) +
  labs(title=paste0("Adjacency Connection: ","mems_shorter")) + 
  xlab("") +
  ylab("")
...
```{r}
mxy <- as.matrix(combined[,c("X","Y")])
mxy%>%
  ggplot(aes(x=X,y=Y))+
  geom_point(color="darkseagreen")+
  theme_minimal()

mxy <- st_as_sf(data.frame(mxy),coords=c("X","Y"),crs=4326, agr = "constant")
st_geometry(mxy)
mst <- spdep::mstree(attr(mems,"listw"))
```
...
for(i in c("st","lt")){
  ...
  for(j in 1:ncol(df_tmp)){
    ...
    list_scalo[[i]][[j]] <- scalogram(df_tmp[,j], mems2)
  }
}
plot(list_scalo$st[[1]])
plot(list_scalo$st[[2]])
