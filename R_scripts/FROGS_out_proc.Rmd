...
###plot seq depth for each datasets

```{r}
plot_seq_depth_all <- ggplot()+
        theme_classic2()
for(i in folder_names){
    
    data_temp <- get(paste0("meco_",gsub("MAPP_","",i)))
    plot_seq_depth_all<- plot_seq_depth_all + 
        geom_point(data=data_temp$sample_table,
                   aes(x=1:(nrow(data_temp$sample_table)),y=reads_post),alpha=.5,shape=20+which(folder_names==i),fill=colorRampPalette(brewer.pal(length(folder_names), "Dark2"))(length(folder_names))[which(folder_names==i)])
}
```
...
data_temp %<>% rename(motus_16s=motus_post,
    ...
    mutate(motus_23s=meco_23s$sample_table$motus_post,
                      reads_23s=meco_23s$sample_table$reads_post,
                      motus_18s=meco_18s$sample_table$motus_post,
...
donuts plot?
```{r}
meco_16s$sample_table%<>%mutate(type="")
ab_trans_16s_donut <- trans_abund$new(meco_16s,taxrank = "Order",ntaxa = 8,groupmean = "type")

order_relab_16s_donut <- ab_trans_16s_donut$plot_donut(labels=NULL)+
    ggtitle("MAPP 16s")+
    theme(plot.title = element_text(face='bold'))+
    theme(plot.tag = element_blank())


meco_18s$sample_table%<>%mutate(type="")
ab_trans_18s_donut <- trans_abund$new(meco_18s,taxrank = "Order",ntaxa = 8,groupmean = "type")

order_relab_18s_donut <- ab_trans_18s_donut$plot_donut(labels=NULL)+
    ggtitle("MAPP 18s")+
    theme(plot.title = element_text(face='bold'))+
    theme(plot.tag = element_blank())


meco_23s$sample_table%<>%mutate(type="")
ab_trans_23s_donut <- trans_abund$new(meco_23s,taxrank = "Order",ntaxa = 8,groupmean = "type")

order_relab_23s_donut <- ab_trans_23s_donut$plot_donut(labels=NULL)+
    ggtitle("MAPP 23s")+
    theme(plot.title = element_text(face='bold'))+
    theme(plot.tag = element_blank(),
...
plot_donuts <- order_relab_16s_donut/order_relab_18s_donut/order_relab_23s_donut
