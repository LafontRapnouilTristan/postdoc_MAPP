...
physeqlists[[folder_names[i]]] <- subset_samples(data_temp, !grepl("23[1-8]|420|241",data_temp@sam_data$sample_id))
}
...
df_ordi_16s <- data.frame(dim1=ordi16s$vectors[,1],
                          ...
                          drought=filtered_16s_hell$sample_table$drought,
                          substrate=filtered_16s_hell$sample_table$substrate
)


pcoa16s_trtmt <- ggplot(df_ordi_16s,aes(x=dim1,y=dim2,color=trtmt))+
  geom_point()+
  theme_classic()+
...
pcoa16s_drought <- ggplot(df_ordi_16s,aes(x=dim1,y=dim2,color=drought))+
  geom_point()+
  theme_classic()+
  ylab(paste0("PCoA 2 (",round(expl2,digits = 3)*100,"%)"))+
  xlab(paste0("PCoA 1 (",round(expl1,digits = 3)*100,"%)"))+
...
pcoa16s_substrate <- ggplot(df_ordi_16s,aes(x=dim1,y=dim2,color=substrate))+
  geom_point()+
  theme_classic()+
  ylab(paste0("PCoA 2 (",round(expl2,digits = 3)*100,"%)"))+
  xlab(paste0("PCoA 1 (",round(expl1,digits = 3)*100,"%)"))+
...
pcoa_comb16s <- (pcoa16s_trtmt|pcoa16s_drought)/(pcoa16s_substrate|plot_spacer())
```
