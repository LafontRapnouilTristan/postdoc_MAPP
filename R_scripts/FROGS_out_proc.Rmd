...
data_set_colors <- c("aquamarine","darkorange","darkorchid4")
...
meco_16s$sample_table$Sample[which(meco_16s$sample_table$Sample=="042047")] <- "203"


meco_23s$sample_table %<>% mutate(Sample = gsub("[A-Z]|-|(?=_).*","",sample_id,perl = T))
meco_23s$sample_table$Sample[which(meco_23s$sample_table$Sample=="205")] <- "203"


setdiff(meco_16s$sample_table$Sample,meco_18s$sample_table$Sample)
...
setdiff(meco_23s$sample_table$Sample,site_coordinates$Sample) 
...
rownames(meco_23s$sample_table) <- meco_23s$sample_table$sample_id
...
### Rarefaction Curves
```{r}
tab16s <- t(otu_table(file2meco::meco2phyloseq(meco_16s)))
class(tab16s) <- "matrix"
curve16s <- rarecurve(tab16s)
names(curve16s) <- rownames(tab16s)
...
for(i in folder_names){
    ...
    n_single_otu <- sum(rowSums(data_temp$otu_table)==1)
    n_non_single_otu <- sum(rowSums(data_temp$otu_table)!=1)
    n_single_reads <- sum(data_temp$otu_table[which(rowSums(data_temp$otu_table)==1),])
    n_non_single_reads <- sum(data_temp$otu_table[which(rowSums(data_temp$otu_table)!=1),])
    df_tmp <- data.frame(data_set=gsub("MAPP_","",i),otu_val=c(n_single_otu,n_non_single_otu),read_val=c(n_single_reads,n_non_single_reads),type=c("n_single","n_non_single"))
    df_tmp %<>% mutate(pct_otus=otu_val/sum(otu_val),
                       pct_reads=read_val/sum(data_temp$otu_table))
    single_list <- rbind(single_list,df_tmp)
}

plot_singletons_reads <- ggplot(single_list,aes(x = data_set,y = pct_reads, fill=type))+
...
plot_singletons_otus <-  ggplot(single_list,aes(x = data_set,y = pct_otus, fill=type))+
           geom_bar(position="stack",stat = "identity")+
    theme_classic2()+
...
We have only singletons in 16s but ~80% of our OTUs
```{r}
df_plot_motu <- data.frame(reads = rowSums(meco_16s$otu_table),
                           motus = rownames(meco_16s$otu_table))
moturead <- ggplot(df_plot_motu,aes(x=log(reads+1),y=reorder(motus,reads)))+
...
ggsave("../Figures/motureads_16s.png",moturead,device = "png")





filtered_16s$otu_table <- filtered_16s$otu_table[which(rowSums(filtered_16s$otu_table)!=1),]
filtered_16s$tidy_dataset()

filtered_its$otu_table <- filtered_its$otu_table[which(rowSums(filtered_its$otu_table)!=1),]
