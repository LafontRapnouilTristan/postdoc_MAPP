...
```{r}
chsn_thrshld <- rep("t_0") #keep 0 as threshold
...
for (i in 1:length(folder_names)){
    ...
    if(target=="16s"){ # Keep all bac and archaea?
        ...
        data_temp$motus$target_taxon <- grepl("Bacteria|Archaea", data_temp$motus$Kingdom)&!grepl("Chloroplast",data_temp$motus$Order)  # MITO and ricketsiales
...
for (i in 1:length(folder_names)){
    ...
    physeqlists[[folder_names[i]]] <- subset_samples(data_temp, !grepl("23[1-7]|239|240|241",data_temp@sam_data$sample_id)) # sample from Vincent's experiments
}
...
class(tab18s) <- "matrix"
curve18s <- rarecurve(tab18s,step=100)
...
class(tab23s) <- "matrix"
curve23s <- rarecurve(tab23s,step=100)
...
We have only singletons in 16s: ~80% of our OTUs but ~4% of our reads.
...
NEED TO CHECK SINGLETONS ASSIGNATIONS
