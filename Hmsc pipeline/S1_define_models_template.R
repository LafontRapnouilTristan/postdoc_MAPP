...
data_sat %<>%
    ...
    select(-matches("_[0-9]{2}$",perl = T))

nb <- missMDA::estim_ncpPCA(data_sat%>%select(where(is.numeric)))
imputed <- missMDA::MIPCA(data_sat%>%select(where(is.numeric)),
               ncp = nb$ncp)
data_sat <- cbind(data_sat[,c("Country","Sample","name")],imputed$res.imputePCA)



Y <- t(meco_23s$otu_table)
rownames(Y)<-  gsub("[A-Z]|-|(?=_).*","",rownames(Y),perl = T)  # get site name (i.e., sample)
samp_to_keep <- rownames(Y)
data_sat %<>% filter(Sample%in%samp_to_keep)

setdiff(rownames(Y),data_sat$Sample)
Y <- Y[!rownames(Y)%in%c("116","119","166"),]


data_sat$Location <- cumsum(!duplicated(data_sat[4:5]))
data_sat %<>% mutate(Sample=as.factor(Sample),
                     Location=as.factor(Location))

rownames(Y)==data_sat$Sample #check if same order
##################################################################################################
...
# XFormula = ~ ...
XFormula <- ~ XData$ph + XData$lai_lt + XData$aet_lt
...
studyDesign <- data.frame(data_sat[,c("Location","Sample")])
...
sData <- as.matrix(distinct(data_sat[,c("X","Y")]))
rownames(sData) <- unique(studyDesign$Location)
rL.location = HmscRandomLevel(sData = sData)
rL.sample = HmscRandomLevel(units = levels(studyDesign$Location))
